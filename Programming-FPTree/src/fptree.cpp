...
bool InnerNode::remove(const Key &k, const int &index, InnerNode *const &parent, bool &ifDelete)
{
    ...
    if (nChild == 1) // 如果只有一个叶子节点(根节点才有的情况)
    {
        bool ifD = false;
        // this->tree->printTree();
        ifRemove = this->childrens[0]->remove(k, 0, this, ifD); // 传递是否删除成功
        // this->tree->printTree();
        if (ifD)
        {
            childrens[0] = NULL;
    ...
    else
    {
        bool ifD = false;
        int temp = findIndex(k);
        ifRemove = this->childrens[temp]->remove(k, temp, this, ifD); // 传递是否删除成功
        if (ifD)                                     // 若孩子节点需要删除
        {
            if (temp == nKeys - 1)
            {
            ...
            if (nKeys < degree && !this->isRoot)
            {
                InnerNode *lb;
                InnerNode *rb;
                this->getBrother(index, parent, lb, rb);
